...
def filter_headlines_with_gemini(google_results, keyword):
    ...
    prompt = (
        ...
        "Return the result as only the headlines in a comma separated array exactly like how python arrays are supposed to be, exactly as how it was before. Just the unrelated headlines.\n\n"
        "Here is the list:\n"
        f"{headlines}"
    )

    response = model.generate_content(prompt)
    print(f"filtering response: {response.text}")
    import json
    try:
