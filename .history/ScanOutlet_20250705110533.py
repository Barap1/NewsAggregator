...
def get_content_threaded(article):
    """Simplified threaded content fetching"""
...
def fetch_articles_content_threaded(filtered_results):
    """Simplified multithreading for content fetching"""
    max_threads = min(10, len(filtered_results))
    print(f"Fetching content using {max_threads} threads...")
    
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        articles_with_content = list(executor.map(get_content_threaded, filtered_results))
    
    print(f"Completed fetching {len(articles_with_content)} articles")
