...
def get_content_threaded(article, index, total_articles):
    """Thread-safe version of get_content with progress tracking"""
    global completed_articles
    
    try:
        # Add delay before each thread runs
        time.sleep(0.5)
        ...
        with progress_lock:
            ...
            print(f"Progress: {completed_articles}/{total_articles} articles processed")
        ...
        with progress_lock:
            global completed_articles
            completed_articles += 1
            print(f"Progress: {completed_articles}/{total_articles} articles processed (Error in article {index})")
...
def fetch_articles_content_threaded(filtered_results):
    """Fetch article content using multithreading with proper locks"""
    global completed_articles
    completed_articles = 0
    
    articles_with_content = []
    max_threads = min(10, len(filtered_results))  # Limit to 10 threads
    
    print(f"Fetching content using {max_threads} threads...")
    
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        # Submit all tasks
        future_to_article = {
            executor.submit(get_content_threaded, article, i, len(filtered_results)): (article, i)
        ...
        for future in as_completed(future_to_article):
            try:
                ...
                article, index = future_to_article[future]
    ...
    # Sort articles by their original index to maintain order
    articles_with_content.sort(key=lambda x: x['index'])
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("fetching contents with multithreading")
        articles_with_content = fetch_articles_content_threaded(filtered_results)
